...
public class Handler {
	...
	public void tick() {
		for (int i = 0; i < object.size(); i++) {
			...
			if (tempObject.getId() == ID.Player || tempObject.getId() == ID.Player2 || tempObject.getId() == ID.Server || tempObject.getId() == ID.Trail || tempObject.getId() == ID.EnemyBurstWarning) {// we don't want these to ever be frozen by the
